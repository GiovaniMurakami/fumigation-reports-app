...
function Detail({ shared = false }) {
  ...
  return <Layout>{!shared && <button className="back" onClick={() => navigate("/")}>← Voltar aos relatórios</button>}<div className="detail-head"><div><div className="lot-list">{item.lotes.map(lote => <span className="lot" key={lote}>LOTE {lote}</span>)}</div><h1>{item.local}</h1><p>{formatDate(item.dataTratamento)}</p></div>{!shared && <button className="primary" onClick={doShare}>↗ Compartilhar</button>}</div>{share && <div className="share-box"><b>Link copiado para a área de transferência</b><input readOnly value={share}/></div>}<section className="detail-card"><h2>Evidências fotográficas</h2><div className="photos">{item.fotos.map(f => <a href={f.url} target="_blank" rel="noreferrer" key={f.chave}><img src={f.url} alt={f.nome}/></a>)}</div></section>{shared && <p className="public-note">Relatório compartilhado por meio de link seguro da Galpex.</p>}</Layout>;
}
